...
Do not make changes to `src/lang/lexer.ts` unless explicitly asked.
Do not make changes to `src/lang/parser.ts` unless explicitly asked.
Interpreter changes are allowed when user explicitly requests expanded language/DOM capabilities.
...
**Web Builder Examples (تطبيقات category):**
- `website-builder` — Full personal website (navbar + hero + skill cards + projects + footer)
